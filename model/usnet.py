...
class USNet(nn.Module):
    def __init__(self, num_classes, backbone_name):
        ...
        ## add mobilenet
            ...
            dilations = [1, 6, 12, 18]
            self.aspp_r = ASPP(320, 256, dilations)
            self.aspp_d = ASPP(320, 256, dilations)

            self.conv_r = ConvBlock(in_channels=256, out_channels=64, kernel_size=1, stride=1, padding=0)
            self.conv_d = ConvBlock(in_channels=256, out_channels=64, kernel_size=1, stride=1, padding=0)

            self.up_r1 = Up_layer(in_channels=96, out_channels=64)
            self.up_r2 = Up_layer(in_channels=32, out_channels=64)
            self.up_r3 = Up_layer(in_channels=24, out_channels=64)

            self.up_d1 = Up_layer(in_channels=96, out_channels=64)
            self.up_d2 = Up_layer(in_channels=32, out_channels=64)
            self.up_d3 = Up_layer(in_channels=24, out_channels=64)
    ...
    def forward(self, input_rgb, input_depth):
        # output of backbone
        x_r1, x_r2, x_r3, x_r4, x_d1, x_d2, x_d3, x_d4 = self.backbone(input_rgb, input_depth, self.backbone_name)
