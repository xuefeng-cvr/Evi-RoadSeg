...
def add_tree_shadow(img, label, tree_shadow):
    ...
    shadow_size = np.where(tree_shadow == 255)[0].shape[0]
    scale = scale * 90 / math.sqrt(shadow_size)
...
def geometry_depth_aug(img, depth, label, cityscapes_cars, instance_bottom_depth, cityscapes_root):
    ...
    cityscapes_imgdepth_mean = (depth * label_erode).sum(1) / (label_erode.sum(1) + 0.00001)
    instance_id1 = np.where(np.min(cityscapes_imgdepth_mean)<instance_bottom_depth)
    instance_id2 = np.where(instance_bottom_depth<np.max(cityscapes_imgdepth_mean))
    ...
    depthdelta = np.abs(cityscapes_imgdepth_mean - instance_bottom_depth[instance_id[random_car]])
    if depthdelta.min() > 5:
        return img, depth, label
    row = np.argmin(depthdelta)

    if np.sum(label[row, :]) < 10:
        return img, depth, label

    index_c = np.where(label[row, :].squeeze() > 0)[0]
