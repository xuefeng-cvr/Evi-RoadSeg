...
from dataset.augmentation_orfd import add_tree_shadow, geometry_depth_aug
import random
...
def orfd_augmentation(img, depth_image, label, shadows, cityscapes_cars, instance_bottom_depth, cityscapes_root):
...
class OrfdAugmentationFusionSegmentation(data.Dataset):
    NUM_CLASSES = 2
    ...
    def __init__(self, args, root='./data/Free/', split='training'):
        ...
        self.image_base = os.path.join(self.root, self.split, '*', 'image_data')
        self.depth_base = os.path.join(self.root, self.split, '*', 'dense_depth')
        self.label_base = os.path.join(self.root, self.split, '*', 'gt_image')
    ...
    def __getitem__(self, index):
        ...
        label_image = cv2.imread(lbl_path, cv2.COLOR_BGR2RGB)
        ...
        if self.split == 'training':
            img_aug, depth_aug, label_aug = orfd_augmentation(img, depth_img, label, self.shadows, self.cityscapes_cars, self.instance_bottom_depth, self.cityscapes_root)
